...
class KyvoLeveling(KyvoBaseCog):
    ...
    def _render_rank_card(
        self, card_color: str, card_bg_color: str, overlay_opacity: float,
        bg_image_bytes: bytes | None, avatar_bytes: bytes | None, avatar_fetch_failed: bool,
        display_name: str, current_level: int, actual_rank: int,
        current_xp: int, xp_needed: int,
    ) -> bytes:
        """랭크 카드 PIL 렌더링 전체(배경 디코딩/합성, 텍스트, 프로그레스 바, 아바타 마스킹) -
        순수 동기 함수로 분리해서 run_in_executor(None, ...)로 이벤트 루프 밖 스레드에서 돌린다.
        네트워크 fetch(aiohttp)는 이미 끝난 bytes만 받으므로 이 함수 자체엔 I/O가 없다."""
        base_w, base_h = 920, 240
        card = None

        if bg_image_bytes is not None:
            try:
                card = fit_card_background(Image.open(io.BytesIO(bg_image_bytes)).convert("RGBA"), (base_w, base_h))
            except Exception as e:
                print(f"[RANK_CARD][WARN] background image decode failed, falling back to solid color: "
        ...
        draw.text((240, 52), display_name, fill="#ffffff", font=self.fonts["name"])
    ...
    async def level(self, interaction: discord.Interaction):
        ...
        # 🛡️ [이벤트 루프 블로킹 수정] 기존엔 await 없이 동기 .execute()를 직접 호출해서 그
        # 왕복 시간만큼 봇 전체의 이벤트 루프가 멈췄다(다른 12개 cog가 이미 쓰는 _db_call로 통일).
        user_override = None
        try:
            override_res = await self._db_call(
                lambda: self.bot.supabase.table("user_card_overrides").select("*")
        ...
        try:
            ...
            response = await self._db_call(
                lambda: self.bot.supabase.table("users").select("user_id", "xp", "level")
        ...
        # 🛡️ [이벤트 루프 블로킹 수정] 네트워크 fetch(aiohttp)는 원래도 non-blocking이라 그대로
        # 두고, bytes만 받아둔다 - 실제 PIL 디코딩/합성(CPU 작업)은 전부 _render_rank_card로
        # 옮겨서 run_in_executor(None, ...)로 이벤트 루프 밖에서 실행한다.
        bg_image_bytes = None
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(background_url, headers=headers, timeout=5) as resp:
                        if resp.status == 200:
                            bg_image_bytes = await resp.read()
                        else:
        ...
        png_bytes = await loop.run_in_executor(
            None, self._render_rank_card,
            card_color, card_bg_color, overlay_opacity, bg_image_bytes, avatar_bytes, avatar_fetch_failed,
            interaction.user.display_name, current_level, actual_rank, current_xp, xp_needed,
        )
        ...
        file = discord.File(fp=io.BytesIO(png_bytes), filename="rank_card.png")
