...
# 🛡️ [재녹음 - 짧은 문장으로 전환] 원래 문장(2~4어절, "-는데요?" 서술형)은 상황을 그대로
# 서술하는 톤이라 캐스터답지 않다는 피드백 + 무음 0곳 게이트 자체도 매번 실패했다(48회 전부
# 실패, 단어 경계마다 생기는 자연스러운 호흡 무음 때문으로 추정). 2~3어절로 줄이니 그제야
# 게이트를 통과했고(a/c는 1~2회 만에, b는 2~3어절 버전이 16회 전부 실패해서 1~2어절로 한
# 번 더 줄인 뒤 1회 만에 통과), 농담기 있는 자연스러운 캐스터 톤도 같이 살렸다.
PRE_BUILDUP_TEXT = {
...
SYSTEM_PROMPT = (
    ...
    "구조 규칙:\n"
    "- 누가 누구를 처치했는지 사실을 요체로 명확하게 전달하는 문장 하나로 만들어라 "
    "(예: '{killer}가 {victim}를 완전히 끝내버렸어요!!').\n"
