...
# 내용은 절대 지어내지 마라" 원칙과 정면으로 어긋남). 위치/챔피언은 물론 "긴장감"/"심상치
# 않음" 같은 상황 판단 자체도 전혀 하지 않는 순수 추임새만 골랐다 - 어떤 클립에 붙어도
# 항상 사실일 수 있는(애초에 아무 판단도 안 하므로) 환각 위험이 없다(예전 BUILDUP_TEXT와
# 동일한 원칙, 이번 라운드에서 한 단계 더 순화됨).
PRE_BUILDUP_POOL = sorted(glob.glob(os.path.join(VOICE_DIR, "pre_buildup_*.wav")))
# 🛡️ [전면 재녹음 - "상황 서술형" -> "순수 추임새"로] "-는데요?" 서술형 문장(2~4어절)은
# 상황을 판단하는 톤이라 캐스터답지 않다는 피드백을 여러 라운드 거치며 계속 줄여왔는데(48회
# 전부 실패 -> 2~3어절 -> 1~2어절), 이번엔 아예 "긴장감"/"심상치 않음"/"뭔가 온다" 같은
# 상황 판단 자체를 다 걷어내고 "음.../어.../오호/흠/허/오"처럼 말을 잇기 전 숨 고르기용
# 순수 감탄사·추임새(1어절 이내)로 전면 교체했다 - 짧을수록 무음 게이트 통과율이 높다는
# 패턴이 이번에도 재확인됨(6개 전부 1~3회 시도 만에 통과, 예전 서술형 문장들의 최대
# 16~48회 대비 압도적으로 안정적).
# 🛡️ [억양 변주 - 비언어적 소리는 이 목소리에서 불안정함이 실측 확인됨] "흠/어/오/우"가 전부
# 1음절이라 서로 구분이 안 된다는 피드백으로, 먼저 비언어적 소리(헛기침/실소/혀 차는 소리)를
# ElevenLabs 태그로 시도했다 - [clears throat]/[chuckles]/[snorts]처럼 태그만 있고 실제
# 단어가 없으면 API가 400(input_text_empty)으로 거부하고, 태그+최소 단어 조합("[clears
# throat] 음" 등 5가지 조합)은 각각 8회씩 시도해도 매번 무음 게이트 실패(비언어적 동작과
# 뒤따르는 단어 사이에 자연스러운 숨쉬기 간격이 생겨서로 추정) - 8/40회 전부 실패해 이
# 목소리로는 안정적 생성이 어렵다고 판단, 폐기. 대신 같은 음소를 유지하되 억양(물음조/
# 단정조/길게 끄는 조)만 바꾸는 대안으로 전환 - 이쪽은 3종 다 1회 시도 만에 통과할 만큼
# 훨씬 안정적이었다. a/c는 기존 그대로 유지, 나머지 4개만 억양을 바꿔 교체(원본은
# _backup_pre_buildup_intonation_*/ 에 백업).
PRE_BUILDUP_TEXT = {
    "pre_buildup_a.wav": "음...",  # 0.48s, 유지(변경 없음)
    "pre_buildup_b.wav": "어?",  # 0.40s, 2회 시도로 무음 0곳 통과 - 물음조로 변경(기존 "어...")
    "pre_buildup_c.wav": "오호",  # 0.72s, 유지(변경 없음)
    "pre_buildup_d.wav": "흠?",  # 0.40s, 1회 시도로 무음 0곳 통과 - 물음조로 변경(기존 "흠")
    "pre_buildup_e.wav": "허...",  # 0.56s, 2회 시도로 무음 0곳 통과 - 끄는조로 변경(기존 "허")
    "pre_buildup_f.wav": "오오",  # 0.88s, 1회 시도로 무음 0곳 통과 - 길게 끄는조로 변경(기존 "오")
}
